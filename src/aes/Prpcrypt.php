 ...
 * @title aes 加解密
 */
namespace normphpCore\encryption\aes;
...
class Prpcrypt
{
    ...
    public function encrypt($text, $appid,$urlencode=true)
    {
        try {
            ...
            if (Helper()->str()->checkcaseOrd($ava_methods[1])){
                $my_method   = 'AES-256-CBC'; # 大写字母
            }else{
                $my_method   = 'aes-256-cbc'; # 小写字母
            }
            ...
            $encrypted = openssl_encrypt($text, $my_method, substr($this->key, 0, 32), OPENSSL_ZERO_PADDING, $iv);
            if($urlencode){

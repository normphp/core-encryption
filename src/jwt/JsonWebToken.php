 ...
 * JSON Web Token基础类
 */
namespace normphpCore\encryption\jwt;
use normphp\config\JsonWebTokenConfig;
use normphpCore\encryption\aes\Prpcrypt;
use normphp\func\Func;
use normphp\helper\ArrayList;
...
class JsonWebToken
{
    ...
    public function decodeJWT(string $jwtString,array $config,\Redis $Redis=null,$TokenSaltName='number')
    {
        ...
        if($Redis){
            ...
            if( !$TokenSalt){throw new \Exception('empty[TokenSalt]',\ErrorOrLog::NOT_LOGGOD_IN_CODE);}
        }
        # 判断签名方法
        if($Header['sig'] == 'md5')
        {
